...
class CameraData(
    ...
    var stream = 0  //输入源图象数据格式（0：YUV420P, 1: NV12，2: NV21） 根据算法定义值  默认 2
    var bMirror = 0   //左右镜象（相对于旋转后的图象，0：左右不镜象，1：左右镜象）
    var nRotate = 0   //旋转方式（对输入源图象旋转，0：不旋转，1：左旋90度，2：右旋90度）
    override fun toString(): String {
        return "CameraData(width=$width, height=$height, CameraID=$CameraID, imageColor=$imageColor, stream=$stream, bMirror=$bMirror, nRotate=$nRotate)"
    }
