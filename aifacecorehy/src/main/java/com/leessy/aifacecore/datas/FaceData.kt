 ...
 */
class FaceData {
    var bitmap: Bitmap? = null//原始位图
    var YUV: ByteArray? = null//yuv

    //    var RGB24: ByteArray? = null//rgb24
    var BGR24: ByteArray? = null//bgr  新版算法那用
    ...
    //    var faceResult: Array<THFI_FacePos>? = null// = arrayListOf(THFI_FacePos())
    var detectResult: THFI_FacePos? = null

    private var rectFaceData: RectData? = null

    fun getRectData(): RectData {
        if (rectFaceData == null) {
            rectFaceData =
                RectData(imageColor, cameraId = CameraID, width = width, height = height).apply {
                    if (detectResult != null)
                        FaceRectEmitterCenter.transf(this, detectResult!!)
                }
        }
        return rectFaceData!!
    }
...
fun FaceData.isReadyCompare(): Boolean {
    return featureGetStatu == 0 && feature != null
            && feature!!.size == THFI_Param.faceFeatureSize
}
